...
const root_route = keys.NEXT_PUBLIC_DOMAIN;
...
module.exports = {
    ...
    collection: {
        ...
        imgSetById: root_route + "/api/collection/imgSet/{id}", // GET, params.id - получить ссылку на картинку по id
        imgSetByName: root_route + "/api/collection/imgSet/name/{name}", // GET,params.name - получить ссылку на картинку по имени
    ...
    product: {
        ...
        read: root_route + "/api/product/",   // GET, params.id - считать один товар по его id
        ...
        // ADDITIONAL
        readOneByName: root_route + "/api/product/name/:name",   // GET, params.name - считать один товар по его имени
        readAll: root_route + "/api/product/",   // GET, - считать все товары (у которых isActive:true)
        readInCollectionById: root_route + "/api/product/inCollection/",   // GET, params.id - считать все товары в коллекции по её id (у которых isActive:true)
        readInCollectionByName: root_route + "/api/product/inCollection/name/:name",   // GET, params.name - считать все товары в коллекции по её имени (у которых isActive:true)
        readFull: root_route + "/api/product/get/full",   // GET, admin - получить все товары, даже те у кого isActive: false
        readByArrIds: root_route + "/api/product/get/byArrIds",  // GET, [json] = { arrIds:["id1", "id2"] } - получить кучу VM товаров по массиву их id
    ...
    sale: {
        ...
        // ADDITIONAL
        readAll: root_route + "/api/sale", // GET - получить все объекты sale
    },
    search: {
    ...
    staticPages: {
        // CRUD
        read: root_route + "/api/staticPage/getContent/:name",   // GET, req.params.name - получить HTML контент статической страницы по её названию (одно из about, delivery, contacts, partnership, qasection, sertificates)
        update: root_route + "/api/staticPage/setContent/",        // POST, admin, form[pageName, content] - записать HTML страницы в БД
